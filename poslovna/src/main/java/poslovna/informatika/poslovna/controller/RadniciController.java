...
*/	
	...
	*/
	...
	public ResponseEntity<Radnik> delete(@PathVariable Long id) {
		List<Radnik> radnici = radniciService.findByMagacin(null);
		boolean obrisati = false;
	...
	public ResponseEntity<List<Radnik>> searchRadnik(@RequestBody Radnik radnik) {
		...
		if(radnik.getMesto() != null){
			radnici= radnici.stream().filter(e->e.getMesto().getNaziv().equals(radnik.getMesto().getNaziv())).collect(Collectors.toList());
		}
	...
	public ResponseEntity<List<Radnik>> editRadnika(@RequestBody Radnik radnikEdit) {
		...
		radnik.setMesto(radnikEdit.getMesto());
